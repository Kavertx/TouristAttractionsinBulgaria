...
class HomeViewModel(
     ...
     */
    suspend fun HashMap<String,String>.retrieveAttractionExtractData(): List<Attraction>{
        val attractionList : MutableList<Attraction> = mutableListOf()
        for ((k,v) in this){
            val response = getAttractionDataWikiRequest(v)
            val attraction = Attraction(
                attractionDistrictId = districtDao.getDistrictId(k),
                attractionName = v,
     ...
     */
    suspend fun Array<String>.retrieveImageUrls(): List<Image>{
        ...
        for (i in this){
            ...
            val imageFileResponse = getAttractionImageFileWikiRequest(i) // contains all img file names
            ...
            imageFileList.forEach{
                val url = getAttractionImageUrlWikiRequest(it).queryImageUrls.imageInfo.values.firstOrNull()?.url
                val img = Image(imageUrl = url.toString(), attractionId = attractionDao.getAttractionId(i))
                imageList.add(img)
            }
        }
        return imageList
     ...
     */
    suspend fun Array<String>.retrieveDistrictExtractData(): List<District>{
